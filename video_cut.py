...
def cut_video(video_path, start, end, label, cfg, output_dir):
    ...
    if duration <= 0:
        logging.warning(f"Skipping invalid cut (duration <= 0): {start} → {end}")
        return

    safe_label = sanitize_filename(label)
    output_file = output_dir / f"{video_path.stem}_{safe_label}.mp4"

    logging.info(f"Cutting: {safe_label} ({start} → {end})")
    ...
    if cfg["mode"] == "fast":
        ...
        bitrate = p["bitrate"]
        bufsize = f"{parse_bitrate(bitrate) * 2}k"
        ...
        cmd = [
            ...
            "-preset", p["preset"],
            "-pix_fmt", "yuv420p10le",
            "-x265-params", f"profile={p['profile']}:level={p['level']}",
...
def main():
    ...
    with open(csv_file, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            start, end, label = row[0], row[1], row[2]
